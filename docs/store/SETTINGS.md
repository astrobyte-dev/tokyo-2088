...
One owner-only beta entry is now verified under Astrobyte. The owner received its actual `apps.garmin.com/apps/…` detail link privately; account-specific URLs are not published here. The listing explicitly directs installation through the mobile Connect IQ app and its Download button is enabled, while the dashboard still labels the entry Pending. Open the actual app-detail link on Android using the same Garmin account in Connect IQ Store and Garmin Connect, then install TOKYO 2088 BETA and sync. If Android remains on the website, use its Connect IQ app link. If the entry cannot open or install, report the exact screen; no alternate public listing or additional upload is authorized. Android opening/install behavior remains unobserved.
