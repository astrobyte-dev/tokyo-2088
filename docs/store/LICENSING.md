...
The compiled delivery mechanism and native reader tests pass. The initial 0.1.0 phone installation/settings report did not validate the separate notice reader. The later 0.1.1 owner report now confirms that **DejaVu / Arev and Noto / SIL OFL both opened and navigated on the physical watch**, and exiting returned to the working face. This records physical navigation, not exhaustive page-by-page hardware inspection or a new legal/rights decision.

The 0.1.1 fix replaces repeated prefix measurement with binary search using the same native font metrics after native interactive opening exposed a watchdog failure. Complete licence resources remain byte-for-byte unchanged. Native simulator first/middle/last-page, boundary and BACK checks remain separately recorded in [validation](VALIDATION.md#local-polish-candidate-011--12-september-2026); the [later owner report](VALIDATION.md#owner-011-update-and-final-presentation-review--12-september-2026) adds physical navigation evidence. No licensing policy, font asset or notice text changed in the presentation pass.
