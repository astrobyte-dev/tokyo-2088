...
Garmin's developer **beta listing is private to the uploading account**. Its URLs are not accessible outside that account, and beta staging requires an alternate manifest app ID. It is useful for the owner's own Store/settings pipeline, **not an invitation-link beta for arbitrary testers**. Official guidance was rechecked for this milestone. `manifest-beta.xml` now supplies the stable separate identity; upload has not occurred because live dashboard/account verification is blocked. [Official beta rules](https://developer.garmin.com/connect-iq/core-topics/beta-apps/)
